...
double mean_squared_error(PPMImage *img1, PPMImage *img2,double &a){
    int size = img1->y*img1->x;
    double loops = 0;
    double temp_I1,temp_I2,E;

    for (int i = 0; i < size; ++i) {

        temp_I1 = (img1->data[i].red + img1->data[i].green + img1->data[i].blue)/3;
        if (a<temp_I1){
            a=temp_I1;
        }
        temp_I2 = (img2->data[i].red + img2->data[i].green + img2->data[i].blue)/3;
        loops+= pow(temp_I1-temp_I2,2);
    }
...
void peak_signal_noise_ratio(PPMImage *img1, PPMImage *img2){
    double A ;
    double E = mean_squared_error(img1,img2,A);
    double psnr = 10* log10(pow(A,2)/ E);
    cout<<"psnr = "+ to_string(psnr)<<endl;
    cout<<"A = "<< A<<endl;
...
int main(int argc, char** argv){
    ...
    peak_signal_noise_ratio(image1,image2);
